...
export default function WorldMapEngine({ seedStr = "world-001", onBack }: WorldMapEngineProps) {
  ...
  const [activeMenuTab, setActiveMenuTab] = useState<'stats' | 'abilities' | 'spells' | 'creation' | 'settings'>('stats');
      ...
      <canvas
        ...
        onKeyDown={(e) => {
          ...
          switch (e.key) {
            ...
            case 'm':
            case 'M':
              console.log('Canvas M key pressed - toggling game menu');
              setShowGameMenu(prev => !prev);
              break;
            case 'Escape':
              if (showGameMenu) {
            ...
            {[
              ...
              <button
                ...
                style={{
                  flex: 1,
                  padding: '12px 16px',
                  background: activeMenuTab === tab.id ? '#374151' : 'transparent',
                  color: activeMenuTab === tab.id ? '#f9fafb' : '#94a3b8',
                  border: 'none',
                  borderBottom: activeMenuTab === tab.id ? '3px solid #3b82f6' : '3px solid transparent',
                  cursor: 'pointer',
                  fontSize: '14px',
                  fontWeight: activeMenuTab === tab.id ? 'bold' : 'normal',
                  transition: 'all 0.2s'
                }}
              >
