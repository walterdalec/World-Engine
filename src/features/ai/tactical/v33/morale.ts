...
export function createMorale(seed: number, base = 60): MoraleState {
...
export function statusOf(value: number): MoraleStatus {
  if (value < 10) return 'Routing';
  if (value < 30) return 'Wavering';
  if (value < 60) return 'Shaken';
  return 'Steady';
}
