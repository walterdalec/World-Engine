 ...
 * 
 * Simulates the entire World Engine ecosystem working together:
 * - World generation with terrain, weather, seasons
 * - Faction AI with diplomatic relations and territorial expansion
 * - Character parties exploring and encountering enemies
 * - Tactical battles on hex grids with terrain effects
 * - Resource management, morale, and progression
 * - Strategic campaign simulation over multiple seasons
 * 
 * This test validates that all systems can coexist and interact correctly.
 */
...
interface Character {
    ...
    mp: number;
    maxMp: number;
    stats: { str: number; dex: number; int: number; con: number; wis: number; cha: number; lck: number };
...
interface Faction {
    ...
    color: string;
    territories: number;
    resources: { gold: number; recruits: number; magic: number };
...
describe('Full Game Integration Simulation', () => {
    it('simulates a complete game session with all systems active', () => {
        ...
        const createTestCharacter = (name: string, archetype: string, level: number): Character => ({
        ...
        for (let season = 0; season < 4; season++) {
            ...
            if (rng.next() > 0.3) { // 70% chance of encounter
                ...
                const allies: BattleUnit[] = playerParty.slice(0, 3).map((char, idx) => ({
                    id: char.id,
                    name: char.name,
                    pos: { q: -2, r: idx },
                    hp: char.hp,
                ...
                const enemies: BattleUnit[] = Array.from({ length: 2 + Math.floor(rng.next() * 3) }, (_, idx) => ({
                    id: `enemy-${idx}`,
                ...
                while (battleActive && battleRounds < 10) {
                    ...
                    const aliveUnits = allUnits.filter((u: BattleUnit) => u.hp > 0);

                    // Calculate damage this round
                    aliveUnits.forEach((unit: BattleUnit) => {
                        const enemies = aliveUnits.filter((u: BattleUnit) => u.team !== unit.team && u.hp > 0);
                        if (enemies.length > 0) {
                    ...
                    const aliveEnemies = allUnits.filter((u: BattleUnit) => u.team === 'enemy' && u.hp > 0).length;
        ...
        expect(simulationResults.seasonsSimulated).toBe(4);
        expect(simulationResults.encountersGenerated).toBeGreaterThan(0);
        expect(integrationScore).toBeGreaterThanOrEqual(60);
    });

    it('validates system data compatibility', () => {
        console.log('\n🔍 System Compatibility Validation\n');

        // Test world noise system
        const noise = new WorldNoise('test-seed');
        const elevation = noise.getElevation(0, 0);
        const temp = noise.getTemperature(0, 0, elevation, 100);
        const moisture = noise.getMoisture(0, 0, elevation);

        console.log('✅ World Noise System: Functional');
        console.log(`   Sample values: elev=${elevation.toFixed(2)}, temp=${temp.toFixed(2)}, moisture=${moisture.toFixed(2)}`);
        ...
        console.log('✅ Seeded RNG System: Functional');
        console.log(`   100 samples average: ${avg.toFixed(3)} (should be ~0.5)`);
