...
export function makeLOSFn(tmap: TerrainMap, opts?: { heightBlockDelta?: number }) {
    const _delta = opts?.heightBlockDelta ?? 2; // blocks if rise > 2 between consecutive steps
    return (h: Axial) => {
...
export function losWithElevation(a: Axial, b: Axial, tmap: TerrainMap, baseBlock: (_h: Axial) => boolean, maxRise = 2) {
