...
export async function generateSimplePortrait(options: SimplePortraitOptions): Promise<PortraitResult> {
    try {
        ...
        console.log(`🎭 Generated portrait: ${gender} ${species} ${archetype}`);
...
function loadAndDrawLayer(
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onerror = () => {
            console.warn(`🎭 Failed to load layer: ${layer.src}`);
            resolve(); // Continue with other layers
        };
